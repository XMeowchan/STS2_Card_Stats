...
$translations = [ordered]@{
    "zh-CN" = [ordered]@{
        ...
        heroBody = "这个页面是 <strong>XiaoHeiHei Card Stats Overlay</strong> 的在线数据入口。Mod 会先读取本地随包数据，有网时再尝试从这里刷新最新的 <code>cards.json</code>。"
    ...
    "en" = [ordered]@{
        ...
        heroTitle = "XiaoHeiHei Card Stats Update Guide"
        heroBody = "This page is the online data entry for <strong>XiaoHeiHei Card Stats Overlay</strong>. The mod reads bundled local data first, then refreshes the latest <code>cards.json</code> from here when a network connection is available."
    ...
    "ja" = [ordered]@{
        ...
        heroTitle = "XiaoHeiHei カード統計 更新ガイド"
        heroBody = "このページは <strong>XiaoHeiHei Card Stats Overlay</strong> のオンラインデータ入口です。Mod はまず同梱されたローカルデータを読み込み、ネット接続があるときにここから最新の <code>cards.json</code> を取得します。"
